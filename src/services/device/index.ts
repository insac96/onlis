import { IDevice, IDeviceConfig } from '../../../types'
import { is_Boolean, is_String, is_Undefined, getObject } from '../../util'
...
export default class Device implements IDevice {
  ...
  private makeFlags () {
    if(!!this.ctxHeaders){
      if (this.userAgent === 'Amazon CloudFront') {
        if (this.ctxHeaders['cloudfront-is-mobile-viewer'] === 'true') {
          this.isMobile = true
          this.isMobileOrTablet = true
        }
        if (this.ctxHeaders['cloudfront-is-tablet-viewer'] === 'true') {
          this.isMobile = false
          this.isMobileOrTablet = true
        }
      }
      else if (this.ctxHeaders['cf-device-type']){
        switch (this.ctxHeaders['cf-device-type']) {
          case 'mobile':
            this.isMobile = true
            this.isMobileOrTablet = true
            break
