...
export default class Device implements IDevice {
  ...
  private makeFlags () {
    if(!!this.headers && this.userAgent === 'Amazon CloudFront'){
      if (this.headers['cloudfront-is-mobile-viewer'] === 'true') {
        this.isMobile = true
        this.isMobileOrTablet = true
      }
      if (this.headers['cloudfront-is-tablet-viewer'] === 'true') {
        this.isMobile = false
        this.isMobileOrTablet = true
      }
    }
    else if (!!this.headers && !!this.headers['cf-device-type']){
      switch (this.headers['cf-device-type']) {
        case 'mobile':
          this.isMobile = true
          this.isMobileOrTablet = true
          break
  ...
  // Init Config
  private initConfig (configs : IConfigs) {
    const deviceConfig : IDeviceConfig = configs.device
    const headers : any = configs.headers

    // Init Device Config
    if(!!deviceConfig){
      const { resize, defaultUserAgent } = deviceConfig

      // Set Resize
      if(!is_Undefined(resize) && !!is_Boolean(resize)){
        this.resize = resize
      }

      // Set Default UserAgent
      if(!!defaultUserAgent && !!is_String(defaultUserAgent)){
